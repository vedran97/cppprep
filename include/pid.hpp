...
namespace prep{
 ...
 struct MotorController{
    ...
    float computeControlAction(const float current_state,const float target_state,const Gains& gains,const float feedforwards=0)noexcept{
        assert(delta_t!=0);
        ...
        auto saturated_integral=std::clamp(integral_action,gains.i_sat_min,gains.i_sat_max);
        auto control_action=proportional_action+saturated_integral+derivative_action +feedforwards;
